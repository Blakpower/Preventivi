...
export const NewQuote: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Generate text listing all items: [Code] Description
    const generatedText = (items || [])
      .filter(item => item && item.description)
      .map(item => `${item.code ? `[${item.code}] ` : ''}${item.description}`)
      .join('\n');
      
    // Update only if field is empty or matches previous auto-generated value (preserving manual edits)
    if (!currentText || currentText === lastAutoGeneratedText.current) {
      if (generatedText !== currentText) {
        setValue('descrizioneProdottiText', generatedText, { shouldDirty: true });
        lastAutoGeneratedText.current = generatedText;
      }
    } else if (!lastAutoGeneratedText.current && currentText && items?.length > 0) {
      // Edge case: Loading an existing quote where text matches what we would generate
  ...
  const onDescriptionChange = (index: number, description: string) => {
    ...
    if (article) {
      const currentCode = getValues(`items.${index}.code`);
      if (!currentCode && article.code) {
